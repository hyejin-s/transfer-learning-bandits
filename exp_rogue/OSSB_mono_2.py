...
class OSSB(BasePolicy):
    ...
    def choice(self):
        ...
        # Monotonize
        if np.all(self.pulls[np.where(means == np.max(means))[0]]<(log_plus(self.t)**2+1)):
        ...
        # undersampled_arms = (np.where(self.pulls <= 2*log_plus(self.t)/log_plus(log_plus(self.t))))[0]
        elif (np.where(self.pulls <= 2*log_plus(self.t)/(1+log_plus(log_plus(self.t)))))[0].size > 0:
            # under-sampled arm
            self.phase = Phase.estimation
            chosen_arm = np.random.choice(np.nonzero(self.pulls == np.min(self.pulls[np.where(self.pulls <= 2*log_plus(self.t)/(1+log_plus(log_plus(self.t))))[0]]))[0])
            ...
            for i in range(self.nbArms):
                if abs(max(means)-means[i])<=gap:
                    means[i] = max(means)
            eta = np.zeros(self.nbArms)
            for i in range(self.nbArms):
                if i in np.where(means == max(means))[0]:
                    eta[i] = min(log_plus(self.t), eta_t[i])
                else:
                    eta[i] = (1+self.gamma)*eta_t[i]
    
            underexplored_arms = (1+self.gamma) * eta * log_plus(self.t) - self.pulls
