...
from environments.Result import *
from environments.MAB import *
...
class Evaluator(object):
    ...
    def __init__(self, configuration,
        ...
        self.embeddings = self.cfg['embeddings']
        ...
        if self.moreAccurate: self.allPulls_rep = dict()
        ...
        for envId in range(len(self.envs)):
            ...
            if self.moreAccurate: self.allPulls_rep[envId] = np.zeros((self.repetitions, self.nbPolicies, self.envs[envId].nbArms, self.horizon), dtype=np.int32)
    ...
    def __initPolicies__(self, env):
        ...
        for policyId, policy in enumerate(self.cfg['policies']):
            ...
            if isinstance(policy, dict):
                ...
                self.policies.append(policy['archtype'](env.nbArms, self.embeddings, **policy['params']))
            else:
            ...
            if hasattr(arm, 'draw_nparray'):  # XXX Use this method to speed up computation
                ...
                for repeatId in tqdm(range(self.repetitions), desc="Repetitions"):
                    for t in tqdm(range(self.horizon), desc="Time steps"):
                        ...
                        # rewards[armId, repeatId, t] = self.envs.draw(armId, t)
        return rewards
    ...
    def startOneEnv(self, envId, env):
        ...
        def store(r, policyId, repeatId):
            ...
            if self.moreAccurate: self.allPulls_rep[envId][repeatId, policyId, :, :] += np.array([1 * (r.choices == armId) for armId in range(env.nbArms)])
            ...
            # if policyId == 1:
            self.estimatedLipschitz[envId][policyId, repeatId] = r.estimated_LC
...
#            self.compareInfo[envId][policyId][repeatId] = r.compare_Info
    ...
    def getRewards_Repe(self, policyId, repeatId, envId=0):
        """Extract mean rewards."""
        return self.rewards_mean[policyId, envId, repeatId, :]
    ...
    def getCumulatedRegret_Repe(self, policyId, repeatId, envId=0):
        return np.cumsum(self.envs[envId].get_maxArm(self.horizon) - self.getRewards_Repe(policyId, repeatId, envId))
    ...
    def getLastRegrets_LessAccurate_Repe(self, policyId, repeatId, envId=0):
        """Extract last regrets, based on accumulated rewards."""
        return np.sum(self.envs[envId].get_maxArm(self.horizon)) - self.lastCumRewards[policyId, envId, repeatId]
    ...
    def plotRegrets(self, filepath, envId=0,
        ...
        labels = [r'$\pi(\infty)$', r'$\pi(\hat{L}_{t})$', r'$\pi(200)$', r'$\pi(0.1)$', 'Thompson', 'UCB', 'klUCB']
        # labels = ['OSSB', 'L:Big', 'trueL']
        for i, policy in enumerate(self.policies):
            if meanReward:
            ...
            if plotSTD and self.repetitions > 1:
                ...
                plt.plot(X, Y, label=labels[i], color=colors[i], marker=markers[i], markevery=(i / 50., 0.1), lw=lw, ms=int(1.5*lw))
        ...
        plt.xticks([0,1000,2000,3000,4000,5000], ['0', '10k', '20k', '30k', '40k', '50k'])
    ...
    def plotRegrets_DEL(self, filepath, name, envId=0,
                    savefig=None, meanReward=False,
        ...
        for i, policy in enumerate(self.policies):
            ...
            if plotSTD and self.repetitions > 1:
                ...
                # std = np.std(Y, axis=0) / np.sqrt(self.repetitions)
                # ci = 1.96 * std
                # print("Y: ", Y)
                mean = [self.getCumulatedRegret_Repe(i, j) for j in range(self.repetitions)]
                # print(mean)
                mean = np.mean(mean, axis=0)
                # print("mean: ", np.mean(mean, axis=0))
                std = np.std(mean, axis=0) / np.sqrt(self.repetitions)
                ci = 1.96 * std
                print(std)
                plt.plot(X, Y, label=labels[i], color=colors[i], marker=markers[i], markevery=(i / 50., 0.1), lw=lw, ms=int(1.5*lw))
                plt.fill_between(X, Y-ci, Y+ci, color=colors[i], alpha=0.1)
        ...
        plt.savefig(filepath+f'/Regret_{name}')
        plt.savefig(filepath+f'/Regret_{name}.pdf', format='pdf', dpi=300)

        return fig

    def RegretsHistory(self, envId=0):
        """Plot the centralized cumulated regret, support more than one environments (use evaluators to give a list of other environments). """
        for i, policy in enumerate(self.policies):
            regrets = []
            for repeatId in range(self.repetitions):
                # last_regret = self.getLastRegrets_LessAccurate_Repe(policyId=i, repeatId=repeatId)
                last_regret = self.getCumulatedRegret_Repe(i, repeatId, envId=0)[-1]
                regrets.append(last_regret)
            print(regrets)
            
    def SaveHistory(self, filepath, envId=0):
        # number of arm pulls
        ...
        with open(filepath+ "/num_ArmPulls.txt", "w") as f:
            ...
            for repeatId in range(self.repetitions):
                f.write('\nRepeat: '+str(repeatId))
                for t in range(self.horizon):
                    cum_pullarm[1][repeatId] += self.allPulls_rep[envId][repeatId, 1, :, t]
                f.write("\n")
                np.savetxt(f, cum_pullarm[1][repeatId].astype(int), fmt='%i', newline=", ")
        ...
        with open(filepath+ "/estimated_Lipschitz_Constant.txt", "w") as f:
            for repeatId in range(self.repetitions):
                ...
                for t in range(self.horizon):
                    #if t % 1000 == 1:
                    f.write('\nt={}, '.format(t))
                    f.write(str(self.estimatedLipschitz[envId][1][repeatId][t]))
            
        with open(filepath+ "/estimated_Lipschitz_Constant_last.txt", "w") as f:
            for policyId in range(len(self.policies)):
                f.write("\n")
                f.write('--- Policy: '+str(policyId))
                save_list = list()
                for repeatId in range(self.repetitions):
                    save_list.append(self.estimatedLipschitz[envId][policyId][repeatId][self.horizon-1])
                f.write("\n")
...
def delayed_play(env, policy, horizon,
    ...
    for t in prettyRange:
        ...
        result.estimated_LC[t] = policy.LC_value
