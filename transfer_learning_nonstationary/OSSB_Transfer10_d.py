...
def get_confusing_bandit(j, d, mu, thetas):
    # theta_max = np.amax(thetas)
    # values : \lambda_i^k (arm k,i \in K^{-})
    lambda_values = np.zeros_like(thetas)
    mu_max = np.max(mu)
    for i, theta in enumerate(thetas):
        lambda_values[i] = max(theta, mu_max/rates[i]- d[i][j])
        if np.all(d == np.inf):
            if j == i:
                lambda_values[i] = mu_max/rates[i] 
        # if np.all(d == np.inf):
        #     if j == i:
        #         lambda_values[i] = mu_max/rates[i]
        # # lambda_values[i] = max(min(theta, mu_max/embeddings[i]+L*abs(embeddings[k]-embeddings[i])), mu_max/embeddings[i]-L*abs(embeddings[k]-embeddings[i]))        
        # else:
        #     lambda_values[i] = max(theta, mu_max/rates[i]- d[i][j])
        
    return lambda_values

def solve_optimization_problem__Lipschitz(mu, thetas, zeta, d, L=-1):
    if L==-1:
        tol = 1e-12
    else:
        tol = 1e-8
    ...
    # c_ge = np.zeros(thetas.size)
    # for i in range(thetas.size):
    #     c_ge[i] = embeddings[i] - mu_max
    
    # sub_arms = (np.where((c_ge>=0) & (c!=0)))[0]
    # opt_arms = (np.where((c_ge<0) | (c==0)))[0]
    ...
    for j, k in enumerate(sub_arms):
        nu = get_confusing_bandit(k, d, mu, thetas) # get /lambda^k
    ...
    delta = c[c!=0]
    # delta = [c[x] for x in sub_arms]
    # delta = c[c!=0 and c_ge>=0]
    # print(delta)

    bounds_sub = np.zeros((sub_arms.size, 2))
    # for idx, i in enumerate(np.where(mu != max(mu))[0]):
...
def solve_optimization_problem__classic(mu, thetas, zeta):
    """ 
    ...
    for i, mu_value in enumerate(mu):
        if mu_value < mu_max: # sub-optimal arm
            values[i] = 1 / klBern(thetas[i], mu_max/rates[i])
        else:
            values[i] = np.inf
    return values
...
class BasePolicy(object):
    ...
    def __init__(self, nbArms, lower=0., amplitude=1.):
        ...
        self.regretRewards = np.zeros(nbArms)
    ...
    def getRegretReward(self, arm, reward):
        """ Give a reward: increase t, pulls, and update cumulated sum of rewards for that arm (normalized in [0, 1])."""
        self.t += 1
        self.pulls[arm] += 1
        reward = (reward - self.lower) / self.amplitude
        # print(reward)
        self.regretRewards[arm] += reward * rates[arm] 
        # self.regretRewards[arm] += reward * embeddings[arm] / 54
...
class OSSB_DEL(BasePolicy):
    ...
    def __init__(self, nbArms, epsilon=EPSILON, gamma=GAMMA,
                 solve_optimization_problem="classic", LC_value=False, distance=False,
        ...
        if solve_optimization_problem == "Lipschitz" and LC_value=="true":
            ...
            self._solve_optimization_problem = solve_optimization_problem__Lipschitz
        if solve_optimization_problem == "Lipschitz" and LC_value=="beta":
            self._info_on_solver = ", Lipschitz, beta"
            self._solve_optimization_problem = solve_optimization_problem__Lipschitz
        if solve_optimization_problem == "Lipschitz" and distance=="true":
            self._info_on_solver = ", distance, true"
            self._solve_optimization_problem = solve_optimization_problem__Lipschitz
        if solve_optimization_problem == "Lipschitz" and distance=="beta":
            self._info_on_solver = ", distance, beta"
            self._solve_optimization_problem = solve_optimization_problem__Lipschitz
    ...
    def getRegretReward(self, arm, reward):
        """ Give a reward: increase t, pulls, and update cumulated sum of rewards for that arm (normalized in [0, 1])."""
        super(OSSB_DEL, self).getRegretReward(arm, reward)
    ...
    def choice(self):
        ...
        means = (self.rewards / self.pulls) # \theta
        meansRegret = (self.regretRewards / self.pulls) # \mu
        ...
        for i in range(self.nbArms):
            if i in np.where(meansRegret == max(meansRegret))[0]:
                zeta[i] = np.inf
            else:
                zeta[i] = zeta_value[i]
                count_undersample += 1
        ...
        global d_matrix
        if self._info_on_solver == ", distance, true":
            d_matrix = self._kwargs['d']
        elif self._info_on_solver == ", distance, beta":
            d_matrix = self._kwargs['d']
        else:
            for i in range(8):
                d_matrix[i] = np.inf        
        ...
        # elif self._info_on_solver == ", Lipschitz, beta":
        #     LCvalue = self._kwargs['L']
        # self.LC_value = LCvalue

        check_sum = np.zeros(self.nbArms)
        sum_cons = np.zeros(count_undersample)
        for idx, i in enumerate(np.where(meansRegret != max(meansRegret))[0]):
            nu_confus = get_confusing_bandit(i, d_matrix, meansRegret, means)
            for k in np.where(meansRegret != max(meansRegret))[0]:
        ...
        ### start
        underSampledArms = np.where(self.pulls <= 10*log_plus(self.t)/log_plus(log_plus(self.t)))[0]
        if underSampledArms.size > 0:
            ...
            bestvalue_arm = np.where(meansRegret == np.max(meansRegret))[0]
            ...
            # for error
            thistime_mt = self._solve_optimization_problem(meansRegret, means, zeta, **self._kwargs)
...
class LipschitzOSSB_DEL_beta(OSSB_DEL):
    def __init__(self, nbArms, gamma=GAMMA, L=betaLC, **kwargs):
        kwargs.update({'L': L})
        super(LipschitzOSSB_DEL_beta, self).__init__(nbArms, gamma=gamma, solve_optimization_problem="Lipschitz", LC_value="beta", **kwargs)
...
class distanceOSSB_DEL_beta(OSSB_DEL):
    def __init__(self, nbArms, gamma=GAMMA, d=betad, **kwargs):
        kwargs.update({'d': d})
        super(distanceOSSB_DEL_beta, self).__init__(nbArms, gamma=gamma, solve_optimization_problem="Lipschitz", distance="beta", **kwargs)

class distanceOSSB_DEL_true(OSSB_DEL):
    def __init__(self, nbArms, gamma=GAMMA, d=trued, **kwargs):
        kwargs.update({'d': d})
        super(distanceOSSB_DEL_true, self).__init__(nbArms, gamma=gamma, solve_optimization_problem="Lipschitz", distance="true", **kwargs)
