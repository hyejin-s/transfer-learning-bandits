...
def get_confusing_bandit(k, L, mu, thetas):
    ...
    for i, theta in enumerate(thetas):
        ...
        lambda_values[i] = max(mu[i]/embeddings[i], (mu_max-L*abs(embeddings[k]-embeddings[i]))/embeddings[i] )
        if L == np.inf:
            if k == i:
                lambda_values[i] = mu_max/embeddings[i]
    return lambda_values

def solve_optimization_problem__Lipschitz(mu, thetas, zeta, L=-1):
    if L==-1:
        tol = 1e-12
    else:
        tol = 1e-8
    ...
    for j, k in enumerate(sub_arms):
        nu = get_confusing_bandit(k, L, mu, thetas) # get /lambda^k
...
def solve_optimization_problem__classic(mu, thetas, zeta):
    ...
    for i, mu_value in enumerate(mu):
        if mu_value < mu_max: #sub-optimal arm
            values[i] = 1 / (klBern(thetas[i], mu_max/embeddings[i])*embeddings[i])
        else:
            values[i] = np.inf
    return values
...
class BasePolicy(object):
    ...
    def getRegretReward(self, arm, reward):
        ...
        # print(reward)
        self.regretRewards[arm] += reward * embeddings[arm] 
...
class OSSB_DEL(BasePolicy):
    ...
    def choice(self):
        ...
        for idx, i in enumerate(np.where(meansRegret != max(meansRegret))[0]):
            nu_confus = get_confusing_bandit(i, LCvalue, meansRegret, means)
