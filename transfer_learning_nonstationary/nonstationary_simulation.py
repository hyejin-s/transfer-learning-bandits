...
from OSSB_Transfer10_theta import OSSB_DEL, LipschitzOSSB_DEL, LipschitzOSSB_DEL_beta, LipschitzOSSB_DEL_true
...
for m in range(M):
    ...
    # mu = np.zeros(numArms)
    # for i in range(numArms):
    #     mu[i] = evaluation.get_lastmeans()[0][0][i]*embeddings[i]
    # defalut_Lipschitz[m] = com_Lipschitz_constant(mu, embeddings)
    # theta
...
for m in range(M):
    ...
    # mu = np.zeros(numArms)
    # for i in range(numArms):
    #     mu[i] = arm_info[m][i]*embeddings[i]
    # trueLC = com_Lipschitz_constant(mu, embeddings)
    # theta    
...
with open(dir_name+ "/L_info.txt", "w") as f:
    for i in range(nbPolicies):
        f.write("\n{} ".format(i))
        if i == 0: # inf
            pass
        elif i == len(POLICIES): # true
            np.savetxt(f, Ltrue_info, newline=" ", fmt='%1.3f')
        else:
            np.savetxt(f, Lbeta_info[i-1], newline=" ", fmt='%1.3f')
