...
class OSSB_DEL(BasePolicy):
    ...
    def choice(self):
        ...
        if underSampledArms.size > 0:
            ...
            for i in range(self.nbArms):
                if i in np.where(means != max(means))[0]:
                    values_c_x_mt2[i] = min((1+self.gamma)*values_c_x_mt[i], log_plus(self.t)*100)
                else:
...
class LipschitzOSSB_DEL_beta(OSSB_DEL):
    def __init__(self, nbArms, gamma=GAMMA, L=betaLC, **kwargs):
        kwargs.update({'L': L})
        super(LipschitzOSSB_DEL_beta, self).__init__(nbArms, gamma=gamma, solve_optimization_problem="Lipschitz", LC_value="true", **kwargs)

class LipschitzOSSB_DEL_true(OSSB_DEL):
    def __init__(self, nbArms, gamma=GAMMA, L=trueLC, **kwargs):
        kwargs.update({'L': L})
        super(LipschitzOSSB_DEL_true, self).__init__(nbArms, gamma=gamma, solve_optimization_problem="Lipschitz", LC_value="beta", **kwargs)
