...
class Evaluator(object):
    ...
    def __init__(self, configuration,
        ...
        self.rewards_mean = np.zeros((self.nbPolicies, len(self.envs), self.horizon, self.repetitions))
        ...
        for envId in range(len(self.envs)):
            ...
            self.compareInfo[envId] = np.zeros((self.nbPolicies, self.repetitions, self.horizon, 5))
    ...
    def getRewards_repe(self, policyId, envId=0):
        """Extract mean rewards."""
        # print(self.rewards_mean[policyId, envId])
        # print(self.rewards_mean[policyId, envId,0])
        # print(self.rewards_mean[policyId, envId,:])
        # print(np.transpose(self.rewards_mean[policyId, envId]))
        return self.rewards_mean[policyId, envId]
    ...
    def plotRegrets_DEL(self, filepath, envId=0,
                    savefig=None, meanReward=False,
        ...
        plot_method = plt.semilogy if semilogy else plot_method
        plot_method = plt.semilogx if semilogx else plot_method
        labels = ['DEL', 'OSSB_DEL', 'Thompson', 'UCB', 'klUCB']
        for i, policy in enumerate(self.policies):
            if meanReward:
                Y = self.getAverageRewards(i, envId)
            else:
        ...
        plt.savefig(filepath+'/Regret_DEL')
        plt.savefig(filepath+'/Regret_DEL.pdf', format='pdf', dpi=300)
    ...
    def plotConditionalExpectation_topbottom(self, filepath, envId=0):
        ...
        plt.xticks(fontsize=40)
        plt.yticks([0,10000,20000,30000,40000,50000], ['0', '10k', '20k', '30k', '40k', '50k'])
        plt.yticks(fontsize=40)
    ...
    def plotLipschitzConstant_tb(self, filepath, envId=0):
        ...
        for i in range(int(self.repetitions*percent)):
            Q[0].append(self.estimatedLipschitz[envId][1][align_ELrepetition[i][0]]) # bottom
            Q[1].append(self.estimatedLipschitz[envId][1][align_ELrepetition[self.repetitions-i-1][0]]) # top
    ...
    def savedata(self, filepath, envId=0, moreAccurate=True):
        ...
        #save regret
        regret = np.zeros((self.repetitions, self.nbPolicies, self.horizon))
        for repeatId in range(self.repetitions):
            for policyId in range(self.nbPolicies):
                regret[repeatId][policyId] += np.cumsum(self.envs[envId].get_maxArm(self.horizon) - np.transpose(self.getRewards_repe(policyId, envId=0))[repeatId])
        print(regret)
        #self.rewards_mean = np.zeros((self.nbPolicies, len(self.envs), self.horizon, self.repetitions))
                        
        compare_regret = np.zeros((self.repetitions, 5))
        with open(filepath+"/action_compareInfo.txt", "w") as f:
            f.write("[initialize, monotonize, estimation, exploitation, exploration]")
            for i, policy in enumerate(self.policies):
                f.write("\nPolicy: " + str(policy) + "\n")
                for repeatId in range(self.repetitions):
                    f.write("\nRepeat: "+str(repeatId))
                    ...
                    for t in range(self.horizon):
                        ...
                        if t == 0:
                            posi = np.where(self.compareInfo[envId][i][repeatId][t] != 0)[0]
                            compare_regret[repeatId][posi] += regret[repeatId][i][t]
                        elif t > 0:
                            posi = np.where((self.compareInfo[envId][i][repeatId][t]-self.compareInfo[envId][i][repeatId][t-1])!=0)[0]
                            compare_regret[repeatId][posi] += regret[repeatId][i][t]-regret[repeatId][i][t-1]
                        f.write("\nt={}, ".format(t))
                        np.savetxt(f, compare_regret[repeatId], newline=", ", fmt='%.4f')
                        f.write("\n")
        
        # with open(filepath+"/action_compareInfo_last.txt", "w") as f:
        #     for i, policy in enumerate(self.policies):
        #         f.write("\nPolicy: " + str(policy) + "\n")
        #         for repeatId in range(self.repetitions):
        #             f.write("\nRepeat: "+str(repeatId))
        #             f.write("\n")
        #             np.savetxt(f, self.compareInfo[envId][i][repeatId][self.horizon-1], newline=", ", fmt='%.4f')
        #             f.write("\n")
        #             np.savetxt(f, compare_regret[repeatId], newline=", ", fmt='%.4f')
        #             f.write("\n")
        """
        ...
        with open(filepath+ "/regret_value.txt", "w") as f:
            for policyId in range(self.nbPolicies):
                f.write('\nPolicy:{}'.format(policyId))
                f.write("\n")
                np.savetxt(f, regret[policyId], fmt='%1.3f', newline=", ")    
